...
summary(PrimacyReg ) # average sleep approaching significance

PrimacySleep <- glm(PrimacyOverall ~ AverageSleep, data=mydata)
summary(PrimacySleep ) # average sleep sig without age groups. 


LorsbachReg <- lm(Lorsbach_Percent ~ Total_Sleep_Disturbance + AverageSleep + AgeGroup, data=mydata)
summary(LorsbachReg) #age but not sleep significant

LorsbachReg <- lm(Lorsbach_Percent ~ Total_Sleep_Disturbance + AverageSleep + AgeGroup, data=mydata)
summary(LorsbachReg)
# 4, 6, 7, 8, age differences significant 
...
LorsbachFull <- lm(Lorsbach_Percent ~  Total_Sleep_Disturbance+AverageSleep, data=fulldata)
summary(LorsbachFull)


PrimacyFull <- lm(All_Primacy ~  AverageSleep, data=fulldata)
summary(PrimacyFull)
#Sleep is signficant in predicting primacy scores.
```
